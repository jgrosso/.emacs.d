...
; Load packages.el first so that everything's installed on a first-time initialization
(load "packages")

; Load utils.el before the rest so that other files can reference it
(load "utils")
